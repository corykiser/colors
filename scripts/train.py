...
from palette.diffusion import VPSchedule, training_step, masked_eps_loss
...
def val_loss(model, sched, batches, device):
    ...
    for b in batches:
        ...
        tot += masked_eps_loss(model(xt, f, m, t, cond), noise, tgt).item()
    model.train()
...
def main():
    ...
    train = subsample(load_arrays(fams, "train", norm), cfg["data"].get("fraction", 1.0), seed)
    val = load_arrays(fams, "val", norm)
    val_raw = load_arrays(fams, "val", None)
    scfg = SamplerConfig(seed=seed, **cfg["sampler"])
    sampler = MaskedCompletionSampler(train, scfg)
    sched = VPSchedule(cfg["diffusion"].get("T", 1000), cfg["diffusion"].get("schedule", "cosine")).to(device)
    ...
    print(f"{name}: {count_params(model):,} params, {len(train):,} train palettes, device={device}", flush=True)
    ...
    for step in range(1, steps + 1):
        ...
        cond = None if b["cond"] is None else {k: v.to(device) for k, v in b["cond"].items()}
        loss = training_step(model, sched, x0, f, m, cond)
    ...
    torch.save({"state_dict": ema.shadow.state_dict(), "config": cfg, "norm": norm.to_dict()}, out / "model_ema.pt")
    ...
    completer = DiffusionCompleter(ema.shadow, sched, norm, device, steps=scfg_s.get("steps", 100), method=scfg_s.get("method", "ddim"),
                                   cfg_scale=scfg_s.get("cfg_scale"), rating=scfg_s.get("rating"))
    ev = evaluate_generator(completer.complete, val_raw.X[val_raw.M.sum(1) == 5], K=scfg_s.get("K", 16), score_fn=load_scorer())
    metrics = {"name": name, "config": cfg, "git": git, "seed": seed, "params": count_params(model), "train_palettes": len(train),
